...
class Table_DW(Table):
    ...
    def reachable_complement(self, flags: 'Flags_DW') -> BoolRef:
        '''reachable method implemented in Table, and here the complement for the Delete Wins Table.'''
        return len(flags.fk_versions) == self.getNumFKs()


    def merge(self, other: 'Table_DW'):
    ...
    def getArgs(extra_id: str, elem: Element, table_size: int, clock: Time ):
        ...
        return Table.getArgs("DW_Table_"+extra_id, elem, table_size, clock, Flags_DW)
